...
public final class PlotEnvelopeCLI extends Application {
    ...
    public final void start(final Stage primaryStage)
        ...
        double[][] cepsLog = new double[times.length][];
        
        double[][] specLog1 = new double[times.length][];
        for(int i = 0; i < times.length; i++) {
        	final int fftSize_ceps = 1 << Le4MusicUtils.nextPow2(cepstrum.length);
        	final int fftSize2_ceps = (fftSize_ceps >> 1) + 1;
        	specLog1[i] = new double[fftSize_ceps];
        	for(int j = 0; j < specLog1[i].length; j++){
        		if(j < specLog[i].length) {
        			specLog[i][j] = specLog[i][j];
        		}
        		else {
        			specLog1[i][j] = 0.0;
        		}
        ...
        for(int i = 0; i <times.length; i++) {
            cepstrum[i] = new Complex[specLog1[i].length];
        	cepstrum[i] = Le4MusicUtils.rfft(specLog1[i]);
           for (int j = 0; j<cepstrum[i].length; j++) {
           	if(j >= 13) {
           		cepstrum[i][j] = new Complex(0,0);
           	}
           }
           cepsLog[i] = Le4MusicUtils.irfft(cepstrum[i]);
        }
        ...
        final NumberAxis yAxis = new NumberAxis(/* axisLabel = */ "Frequency (Hz)", /* lowerBound = */ 0.0,
                /* upperBound = */ 500, /* tickUnit = */ Le4MusicUtils.autoTickUnit(300));
        ...
        chart.setTitle("Spectrogram" + " " + pargs[0]);
        Arrays.stream(specLog).forEach(chart::addSpecLog);
//        Arrays.stream(cepsLog).forEach(chart::addSpecLog);

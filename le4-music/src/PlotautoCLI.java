...
public final class PlotautoCLI extends Application {
   ...
   @Override
    public final void start(final Stage primaryStage)
        ...
        double[] autoCorrelation = new double[times.length];
        for(int i = 0; i < times.length; i++) { //frame
        	double[] ac = new double[specLog2[i].length];
        	for(int tau = 0; tau < specLog2[i].length; tau++) { //tau
        		for(int j = 0; j <= specLog2[i].length - tau - 1; j++) {
        			if(j - tau >= 0 && j - tau < specLog2[i].length) {
        				ac[tau] += specLog[i][j] * specLog2[i][j - tau];		
        			}
        		}
        	}
        	double min_2 = ac[0];
        	int pointer = 0;
        	for(int t = 1; t < ac.length; t++) { //t ==  then ac[tau] is max
        		if(ac[t] > min_2 && ac[t] != 0) {
        			min_2 = ac[t];
        			pointer = t;
        		}
        	}
        	...
        	if(pointer == 0) {
        		...
        		autoCorrelation[i] = 1 / min_2;	
        	}
        	System.out.println(autoCorrelation[i] * sampleRate / fftSize);
        }

        
        /* データ系列を作成*/
        final ObservableList<XYChart.Data<Number, Number>> data =
            IntStream.range(0, autoCorrelation.length)
                    .mapToObj(i -> new XYChart.Data<Number, Number>(i * shiftDuration, autoCorrelation[i] * sampleRate / fftSize))
        ...
        final NumberAxis yAxis = new NumberAxis(/* axisLabel = */ "Frequency (Hz)", /* lowerBound = */ 0.0,
                /* upperBound = */ 1000, /* tickUnit = */ Le4MusicUtils.autoTickUnit(300));

...
public final class PlotYesNoCLI extends Application {
   ...
   @Override
    public final void start(final Stage primaryStage)
        ...
        double[] f0 = new double [25]; // 25 = waveform.length / frameSize
        double[] new_freq = new double [25];
        ...
        for(int i = 0; i < new_freq.length; i ++) {
        	...
        	for(int j = 0; j < frameSize-1; j++) {
        		if(waveform[i * frameSize + j] * waveform[i * frameSize + j + 1] < 0) {
        			zero_cross += 1;
        		}
        	}
        	...
        	System.out.print(i + " : " + new_freq[i] * sampleRate / fftSize);
        	
        }
        
        /* データ系列を作成*/
        final ObservableList<XYChart.Data<Number, Number>> data =
            IntStream.range(0, new_freq.length)

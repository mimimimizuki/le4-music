...
public final class PlotaiueoCLI extends Application {
    ...
    @Override public final void start(final Stage primaryStage)
        ...
        * 振幅を信号長で正規化する． */
        ...
        Complex[] cepstrum = Le4MusicUtils.fft(specLog);
        ...
        * freqs[fftSize2 - 1] = sampleRate / 2 (= Nyquist周波数) */
        ...
        final double[] aiueoceps = Arrays.stream(ceceps).mapToDouble(c -> c.abs()).toArray();
        
        System.out.println(ceceps.length);
        int sum = 0;
        for(int i = 0; i < aiueoceps.length; i ++) {
        	sum += aiueoceps[i];
        }
        int m = sum / aiueoceps.length; //最尤推定した結果の平均
        sum = 0;
        for(int i = 0; i < aiueoceps.length; i ++) {
        	sum += (aiueoceps[i] - m) * (aiueoceps[i] - m);
        }
        int s_power = sum / aiueoceps.length; //最尤推定した結果の分散
        
        System.out.print("average is " + m +", " + "bunsan is " + s_power);
